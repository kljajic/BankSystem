...
public interface CurrencyRepository extends JpaRepository<Currency, Long> {
	...
	@Query(value="select distinct c.id, c.name, c.official_code, c.country_id, c.domicilna from currency c inner join country d on c.country_id = d.id where c.name like ?1% and c.official_code like ?2% and c.domicilna = ?4 and d.name like ?3%" ,nativeQuery=true)
